...
int bank(int atm_out_fd[], Command *cmd, int *atms_remaining) {
  ...
  int result = SUCCESS;
   //int write = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE);
  // TODO: your code here 
  result = (check_valid_atm(i) != SUCCESS) ? ERR_UNKNOWN_ATM : result; 
  if(result != SUCCESS) { return ERR_UNKNOWN_ATM; }
  if(c == CONNECT) {
    MSG_OK(&bankcmd, 0, f, t, a);
    result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE);
  } else if(c == EXIT) {
    (*atms_remaining) = (*atms_remaining) - 1;
    MSG_OK(&bankcmd, 0, f, t, a);
    result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE);
  } else if(c == DEPOSIT) {
    (check_valid_account(t) != SUCCESS) ? (MSG_ACCUNKN(&bankcmd, 0, t), result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE)) : 
      (accounts[t] += a, MSG_OK(&bankcmd, 0, f, t, a), result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE));
  } else if(c == WITHDRAW) { 
    (check_valid_account(f) != SUCCESS) ? (MSG_ACCUNKN(&bankcmd, 0, f), result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE)) : 
      (accounts[f] < a) ? (MSG_NOFUNDS(&bankcmd, 0, f, a), result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE)) 
                          : (MSG_OK(&bankcmd, 0, f, t, a), result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE));
  } else if(c == TRANSFER) {
    (check_valid_account(t) != SUCCESS) ? (MSG_ACCUNKN(&bankcmd, 0, t), result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE)) : 
      (check_valid_account(f) != SUCCESS) ? (MSG_ACCUNKN(&bankcmd, 0, f), result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE)) : 
        (accounts[f] < a) ? (MSG_NOFUNDS(&bankcmd, 0, f, a), result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE)) : 
    ...
    (check_valid_account(f) != SUCCESS) ? (MSG_ACCUNKN(&bankcmd, 0, f), result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE)) : 
      (MSG_OK(&bankcmd, 0, f, t, accounts[f]), result = checked_write(*atm_out_fd, &bankcmd, MESSAGE_SIZE));
  } else {
     (error_msg(ERR_UNKNOWN_CMD, "unknown error")); 
