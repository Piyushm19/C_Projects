...
int atm(int bank_out_fd, int atm_in_fd, int atm_id, Command *cmd) {
  ...
  status = (c == OK) ? status = SUCCESS : (c == NOFUNDS) ? status = ERR_NOFUNDS: (c == ACCUNKN) ? 
                       status = ERR_UNKNOWN_ACCOUNT : (error_msg(ERR_UNKNOWN_CMD, "received unknown commands"), status = ERR_UNKNOWN_CMD); 
  return status;
}
